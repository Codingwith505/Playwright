...
test.beforeAll('login through ui and store in json ', async({ browser }) =>
{
    ...
    // use the store json data to login new browser with json creadential
    webApiContext = await browser.newContext({storageState: 'state.json'});
});
...
test('Page playwright test', async () => {
    ...
    // login with json creadential
    const page = await webApiContext.newPage();
    await page.goto("https://rahulshettyacademy.com/client/");
    ...
    const cleanId1 = await orderId.trim();
    const cleanId = await cleanId1.split("|")[1].trim();
    console.log(cleanId);
